  ...
  return (
    <div className="flex gap-3 flex-wrap pt-1">
      {checks.map((c) => (
        <span
          key={c.label}
          className={`flex items-center gap-1 text-xs transition-colors ${c.ok ? "text-primary" : "text-muted-foreground"}`}
        >
          <Check className={`w-3 h-3 ${c.ok ? "text-primary" : "text-muted-foreground/40"}`} />
          {c.label}
        </span>
      ))}
    </div>
...
export default function SignupPage() {
  ...
  return (
    <motion.div {...fadeUp(0)} className="min-h-screen flex items-center justify-center px-4 md:py-18 bg-background">
      <div className="w-full max-w-md">
        <div className="mb-2 text-center">
          ...
          <h1 className="text-2xl font-bold text-foreground tracking-tight">{t("signup.title")}</h1>
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm p-8">
          {submitted ? (
            <div className="text-center py-4">
              ...
              <h2 className="text-lg font-semibold text-foreground">{t("signup.account_created")}</h2>
            ...
            <form onSubmit={formik.handleSubmit} noValidate className="space-y-4">
              ...
              >
                {formik.isSubmitting ? (
                  <><Loader2 className="w-4 h-4 animate-spin" /> {t("signup.creating_account")}</>
                ) : (
